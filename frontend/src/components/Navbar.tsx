      ...
      <div className="w-full px-6 sm:px-10 lg:px-16">
        <div className={`relative flex items-center justify-between transition-all duration-400 ${scrolled ? 'h-14' : 'h-16 lg:h-[65px]'}`}>
          ...
          <div className="hidden lg:flex items-center gap-2 absolute left-1/2 -translate-x-1/2">
            {['Find Jobs', 'AI Features', 'About', 'FAQ'].map(item => (
              <a
                ...
                className={`group relative px-4 py-2 text-sm font-medium transition-all duration-200 rounded-lg hover:scale-[1.02] ${
                  isDark
          ...
          <div className="flex items-center gap-3 z-10">
            ...
            <button
              onClick={toggleTheme}
              className={`relative w-14 h-8 rounded-full p-1 transition-all duration-500 cursor-pointer ${
            ...
            {isSignedIn ? (
              ...
              <div className="relative" ref={profileRef}>
                <button
                  ...
                  className={`group relative w-10 h-10 rounded-full bg-gradient-to-tr from-primary-600 via-primary-500 to-amber-500 p-[2px] shadow-md transition-all duration-300 cursor-pointer hover:scale-105 ${
                    profileOpen ? 'ring-2 ring-primary-400 ring-offset-2 ring-offset-transparent' : 'hover:shadow-primary-500/30'
                  }`}
                  aria-label="User Profile"
                >
                  <div className={`w-full h-full rounded-full flex items-center justify-center font-bold text-sm transition-colors ${
                    isDark ? 'bg-[#0a0e2a] text-white' : 'bg-white text-primary-600'
                  }`}>
                ...
                {/* Profile Dropdown */}
                {profileOpen && (
                  <div className={`absolute right-0 mt-2 w-52 rounded-xl overflow-hidden shadow-2xl border animate-slide-up z-50 ${
              ...
              <div className="hidden lg:flex items-center gap-2">
                <button
                  onClick={openLogin}
                  className={`px-3.5 sm:px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 cursor-pointer ${
          ...
          {!isSignedIn && (
            <div className="pt-3 flex gap-3 border-t mt-2">
              <button
                ...
                className={`flex-1 text-center py-2.5 rounded-xl text-sm font-semibold border cursor-pointer transition-colors ${
                  isDark ? 'border-[#1a1f45] text-surface-300 hover:bg-white/5' : 'border-surface-300 text-surface-600 hover:bg-surface-50'
                }`}
              >
                Sign In
              </button>
