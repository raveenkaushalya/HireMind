    ...
    <div className={`min-h-screen ${isDark ? 'bg-surface-950' : 'bg-surface-50'}`}>
      ...
      <div className="w-full px-6 sm:px-10 lg:px-16 py-6 space-y-6">
        ...
        <div className={`rounded-xl p-2 flex items-center gap-2 ${
          isDark ? 'bg-surface-800/80 border border-surface-700/50' : 'bg-white border border-surface-200 shadow-sm'
        }`}>
          ...
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className={`text-xs px-3 py-1 rounded-lg cursor-pointer ${
            ...
            <div key={category}>
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                  isDark ? 'bg-surface-800 text-surface-400' : 'bg-surface-200 text-surface-500'
              ...
              <div className="grid lg:grid-cols-2 gap-4 mb-8">
                {categoryJobs.map(job => (
                  <div
                    ...
                    className={`group relative rounded-2xl p-5 transition-all duration-200 hover:-translate-y-0.5 cursor-pointer ${
                      isDark
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-3 mb-1.5">
                          <div>
                            <h3 className={`font-display font-bold text-sm group-hover:text-primary-500 transition-colors ${
                              isDark ? 'text-white' : 'text-surface-900'
                            }`}>{job.title}</h3>
                            <p className={`text-xs font-medium ${isDark ? 'text-surface-400' : 'text-surface-500'}`}>{job.company}</p>
                          </div>
                          <div className={`relative w-10 h-10 rounded-lg bg-gradient-to-br ${
                            job.aiScore >= 95 ? 'from-emerald-400 to-emerald-600' :
